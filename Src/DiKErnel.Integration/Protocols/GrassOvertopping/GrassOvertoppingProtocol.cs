...
namespace DiKErnel.Integration.Protocols.GrassOvertopping
{
    internal class GrassOvertoppingProtocol : Protocol
    {
                                      ...
                                      IProfileData profileData)
        {
            ...
            double calculatedDikeHeight = CalculateDikeHeight(locationDependentInput.X, outerCrest,
                                                              profileData.ProfileSegments,
                                                              profileData.GetVerticalHeight(locationDependentInput.X));
        ...
        protected override void InitializeDerivedLocationDependentInput(ILocationDependentInput locationDependentInput,
                                                                        IProfileData profileData)
        {
            base.InitializeDerivedLocationDependentInput(locationDependentInput, profileData);
            ...
            InitializeDikeHeight(locationDependentInput.X, outerCrest, profileData.ProfileSegments);
            InitializeAccelerationAlphaA(locationDependentInput.X, outerCrest, innerCrest);
        }
            ...
            if (verticalDistanceWaterLevelElevation >= 0.0)
            {
                ...
                if (RevetmentFunctions.FailureRevetment(damage, initialDamage, locationDependentInput.FailureNumber))
                {
                    double durationInTimeStepFailure = RevetmentFunctions.DurationInTimeStepFailure(
                        incrementTime, incrementDamage, locationDependentInput.FailureNumber, initialDamage);
        ...
        private void InitializeDikeHeight(double x, (double, double) outerCrest, IReadOnlyList<ProfileSegment> profileSegments)
        {
            dikeHeight = CalculateDikeHeight(x, outerCrest, profileSegments, Z);
        }

        private void InitializeAccelerationAlphaA(double x, (double, double) outerCrest, (double, double) innerCrest)
        {
        ...
        private double CalculateDikeHeight(double x, (double, double) outerCrest,
