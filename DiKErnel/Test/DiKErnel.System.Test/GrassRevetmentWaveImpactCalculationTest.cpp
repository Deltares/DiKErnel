...
#pragma region Schematization 3
	...
	TEST_F(GrassRevetmentWaveImpactCalculationTest, PerformanceTest)
	{
		...
		ofstream MyFile("PerformanceTestGrassRevetmentWaveImpactCalculationTest.txt", ios::out | ios::trunc);
		...
		MyFile << "Total Calculation time for GrassRevetmentWaveImpactCalculationTest is: " << ms_double.count() << " milli seconds" << "\n";
		MyFile << "Average calculation time per GrassRevetmentWaveImpactCalculationTest is: " << ms_double.count() / (item * TestDefaults::GetNumberOfTestRuns()) << " milli seconds" << "\n";
		MyFile.close();
	}
